/*
largest product in a grid, including left to right, up to down, diagonal left and right
*/
#include <stdio.h>
#include <stdint.h>
#include <stdbool.h>
...
int main()
{
    ...
    uint64_t multi_val = 1;
    // check for left to right
    for (int i = 0; i < GRID_SIZE; i++)
    {
        for (int j = 0; j <= (GRID_SIZE - ADJACENT_NUMBERS); j++)
        {
            for (int m = 0; m < ADJACENT_NUMBERS; m++)
            {
                multi_val *= grid[i][m + j];
                printf("grid i m j: %d\n", grid[i][m + j]);
            }
            printf("multi = %lld \n", multi_val);
            if (multi_val > largest_val)
                largest_val = multi_val;
            multi_val = 1;
        }
    }
...
/*
Execution time: 4.6610298 seconds
Solution date: 2024-11-11
results: 70600674
THE END
*/